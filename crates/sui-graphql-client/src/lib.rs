...
mod tests {
    ...
    async fn test_total_transaction_blocks() {
        ...
        let total_transaction_blocks_by_chckp =
            client.total_transaction_blocks_by_seq_num(chckp_id).await;
        let blocks_chckp = total_transaction_blocks_by_chckp.unwrap().unwrap();
        assert!(blocks_chckp > 0);
        ...
        assert_eq!(total_transaction_blocks, blocks_chckp);
    }
